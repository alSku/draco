...
bool SplitPath(const std::string &full_path, std::string *out_folder_path, std::string *out_file_name)
{
...
std::string ReplaceFileExtension(const std::string &in_file_name, const std::string &new_extension)
{
...
std::string GetFullPath(const std::string &input_file_relative_path, const std::string &sibling_file_full_path)
{
...
{
  std::unique_ptr<FileReaderInterface> file_reader = FileReaderFactory::OpenReader(file_name);
...
bool ReadFileToBuffer(const std::string &file_name, std::vector<uint8_t> *buffer)
{
  std::unique_ptr<FileReaderInterface> file_reader = FileReaderFactory::OpenReader(file_name);
...
bool WriteBufferToFile(const char *buffer, size_t buffer_size, const std::string &file_name)
{
  std::unique_ptr<FileWriterInterface> file_writer = FileWriterFactory::OpenWriter(file_name);
...
bool WriteBufferToFile(const unsigned char *buffer, size_t buffer_size, const std::string &file_name)
{
  return WriteBufferToFile(reinterpret_cast<const char *>(buffer), buffer_size, file_name);
}

bool WriteBufferToFile(const void *buffer, size_t buffer_size, const std::string &file_name)
{
  return WriteBufferToFile(reinterpret_cast<const char *>(buffer), buffer_size, file_name);
}
