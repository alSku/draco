...
std::unique_ptr<CornerTable> CornerTable::Create(const IndexTypeVector<FaceIndex, FaceType> &faces)
{
...
bool CornerTable::ComputeOppositeCorners(int *num_vertices)
{
  ...
  {
    VertexEdgePair() : sink_vert(kInvalidVertexIndex), edge_corner(kInvalidCornerIndex)
    {
    }
  ...
  for (CornerIndex c(0); c < num_corners(); ++c)
  {
    ...
    if (opposite_c == kInvalidCornerIndex)
    {
      ...
      const int num_corners_on_source_vert = num_corners_on_vertices[source_v.value()];
...
bool CornerTable::BreakNonManifoldEdges() {
  ...
  do
  {
    ...
    for (CornerIndex c(0); c < num_corners(); ++c)
    {
      ...
      while (next_c = SwingLeft(current_c), next_c != first_c && next_c != kInvalidCornerIndex && !visited_corners[next_c.value()])
        current_c = next_c;
      ...
      do
      {
        ...
        for (auto &&attached_sink_vertex : sink_vertices)
        {
          if (attached_sink_vertex.first == sink_v)
          {
            ...
            const CornerIndex opp_other_edge_corner = Opposite(other_edge_corner);
