...
bool MeshAttributeCornerTable::InitFromAttribute(const Mesh *mesh, const CornerTable *table, const PointAttribute *att)
{
  if (!InitEmpty(table))
    return false;
  ...
  for (CornerIndex c(0); c < corner_table_->num_corners(); ++c)
  {
    ...
    for (int i = 0; i < 2; ++i)
    {
      ...
      const PointIndex sibling_point_id = mesh->CornerToPointId(act_sibling_c.value());
      ...
      if (att->mapped_index(point_id) != att->mapped_index(sibling_point_id))
      {
        ...
        // Mark seam vertices.
        is_vertex_on_seam_[corner_table_->Vertex(corner_table_->Next(CornerIndex(c))).value()] = true;
        is_vertex_on_seam_[corner_table_->Vertex(corner_table_->Previous(CornerIndex(c))).value()] = true;
        is_vertex_on_seam_[corner_table_->Vertex(corner_table_->Next(opp_corner)).value()] = true;
        is_vertex_on_seam_[corner_table_->Vertex(corner_table_->Previous(opp_corner)).value()] = true;
        break;
      }
    }
  }
...
void MeshAttributeCornerTable::AddSeamEdge(CornerIndex c)
{
  ...
  // Mark seam vertices.
  is_vertex_on_seam_[corner_table_->Vertex(corner_table_->Next(c)).value()] = true;
  is_vertex_on_seam_[corner_table_->Vertex(corner_table_->Previous(c)).value()] = true;
...
void MeshAttributeCornerTable::RecomputeVertices(const Mesh *mesh, const PointAttribute *att)
{
  DRACO_DCHECK(GetValenceCache().IsCacheEmpty());
...
void MeshAttributeCornerTable::RecomputeVerticesInternal(const Mesh *mesh, const PointAttribute *att)
{
  DRACO_DCHECK(GetValenceCache().IsCacheEmpty());
