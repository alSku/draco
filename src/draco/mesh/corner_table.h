...
//    std::cout << "Corner: " << corner.value() << std::endl;
//    std::cout << "corner to vertex map size: " << corner_to_vertex_map_.size() << std::endl;
//
//    auto* p = corner_to_vertex_map_.data();
//
//    std::cout << "corner_to_vertex_map_ contains:";
//
//    for (CornerIndex c(0); c < corner_to_vertex_map_.size(); ++c)
//      std::cout << ' ' << corner_to_vertex_map_[c];
//
//    std::cout << '\n';
  ...
  inline void SetOppositeCorner(CornerIndex corner_id, CornerIndex opp_corner_id)
  {
    DRACO_DCHECK(GetValenceCache().IsCacheEmpty());
  ...
  FaceIndex AddNewFace(const std::array<VertexIndex, 3> &vertices)
  {
    ...
    for (int i = 0; i < 3; ++i)
    {
      ...
      SetLeftMostCorner(vertices[i],CornerIndex(corner_to_vertex_map_.size() - 1));
    }
...
static constexpr CornerTable::FaceType kInvalidFace({{kInvalidVertexIndex, kInvalidVertexIndex, kInvalidVertexIndex}});
