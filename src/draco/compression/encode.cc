...
Status Encoder::EncodePointCloudToBuffer(const PointCloud &pc, EncoderBuffer *out_buffer)
{
...
EncoderOptions Encoder::CreateExpertEncoderOptions(const PointCloud &pc) const
{
  ...
  {
    const Options *att_options = options().FindAttributeOptions(pc.attribute(i)->attribute_type());
...
void Encoder::Reset(const EncoderOptionsBase<GeometryAttribute::Type> &options)
{
  Base::Reset(options);
}
...
void Encoder::SetAttributeQuantization(GeometryAttribute::Type type, int quantization_bits)
{
...
Status Encoder::SetAttributePredictionScheme(GeometryAttribute::Type type, int prediction_scheme_method)
{
  ...
  options().SetAttributeInt(type, "prediction_scheme", prediction_scheme_method);
  return status;
}
