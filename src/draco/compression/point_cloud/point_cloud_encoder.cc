...
Status PointCloudEncoder::Encode(const EncoderOptions &options, EncoderBuffer *out_buffer)
{
  options_ = &options;
  buffer_ = out_buffer;
...
Status PointCloudEncoder::EncodeMetadata()
{
  ...
  if (!metadata_encoder.EncodeGeometryMetadata(buffer_,point_cloud_->GetMetadata()))
...
bool PointCloudEncoder::EncodePointAttributes()
{
  ...
  for (int att_encoder_id : attributes_encoder_ids_order_)
  {
    if (!attributes_encoders_[att_encoder_id]->EncodeAttributesEncoderData(buffer_))
      return false;
  }
...
bool PointCloudEncoder::MarkParentAttribute(int32_t parent_att_id)
{
  ...
  const int32_t parent_att_encoder_id = attribute_to_encoder_map_[parent_att_id];
  return attributes_encoders_[parent_att_encoder_id]->MarkParentAttribute(parent_att_id);
}
...
const PointAttribute *PointCloudEncoder::GetPortableAttribute(int32_t parent_att_id)
{
  ...
  const int32_t parent_att_encoder_id = attribute_to_encoder_map_[parent_att_id];
  return attributes_encoders_[parent_att_encoder_id]->GetPortableAttribute(parent_att_id);
}
...
bool PointCloudEncoder::RearrangeAttributesEncoders()
{
  ...
  while (num_processed_encoders < attributes_encoders_.size())
  {
    ...
    for (uint32_t i = 0; i < attributes_encoders_.size(); ++i)
    {
      ...
      for (uint32_t p = 0; p < attributes_encoders_[i]->num_attributes(); ++p)
      {
        ...
        for (int ap = 0; ap < attributes_encoders_[i]->NumParentAttributes(att_id); ++ap)
        {
          const uint32_t parent_att_id = attributes_encoders_[i]->GetParentAttributeId(att_id, ap);
          const int32_t parent_encoder_id = attribute_to_encoder_map_[parent_att_id];
    ...
    if (!encoder_processed && num_processed_encoders < attributes_encoders_.size())
    {
  ...
  std::vector<bool> is_attribute_processed(point_cloud_->num_attributes(),false);
  int num_processed_attributes;

  for (uint32_t ae_order = 0; ae_order < attributes_encoders_.size(); ++ae_order)
  {
    const int ae = attributes_encoder_ids_order_[ae_order];
    const int32_t num_encoder_attributes = attributes_encoders_[ae]->num_attributes();
    ...
    while (num_processed_attributes < num_encoder_attributes)
    {
      ...
      for (int i = 0; i < num_encoder_attributes; ++i)
      {
        ...
        for (int p = 0; p < attributes_encoders_[ae]->NumParentAttributes(att_id); ++p)
        {
          const int32_t parent_att_id = attributes_encoders_[ae]->GetParentAttributeId(att_id, p);
      ...
      if (!attribute_processed && num_processed_attributes < num_encoder_attributes)
      {
