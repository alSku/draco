...
template <class TraversalEncoder>
bool MeshEdgebreakerEncoderImpl<TraversalEncoder>::Init(MeshEdgebreakerEncoder *encoder)
{
  encoder_ = encoder;
  mesh_ = encoder->mesh();
  ...
  {
    use_single_connectivity_ = encoder_->options()->GetGlobalBool("split_mesh_on_seams", false);
  }
...
template <class TraversalEncoder>
const MeshAttributeCornerTable* MeshEdgebreakerEncoderImpl<TraversalEncoder>::GetAttributeCornerTable(int att_id) const
{
...
template <class TraversalEncoder>
const MeshAttributeIndicesEncodingData* MeshEdgebreakerEncoderImpl<TraversalEncoder>::GetAttributeEncodingData(int att_id) const
{
  for (uint32_t i = 0; i < attribute_data_.size(); ++i)
  {
    if (attribute_data_[i].attribute_index == att_id)
...
std::unique_ptr<PointsSequencer>
    MeshEdgebreakerEncoderImpl<TraversalEncoder>::CreateVertexTraversalSequencer(MeshAttributeIndicesEncodingData *encoding_data)
{
  ...
  AttObserver att_observer(corner_table_.get(), mesh_, traversal_sequencer.get(), encoding_data);
...
template <class TraversalEncoder>
bool MeshEdgebreakerEncoderImpl<TraversalEncoder>::GenerateAttributesEncoder(int32_t att_id)
{
  ...
  const int32_t element_type = GetEncoder()->mesh()->GetAttributeElementType(att_id);
  const PointAttribute *const att = GetEncoder()->point_cloud()->attribute(att_id);
  ...
  if (use_single_connectivity_ || att->attribute_type() == GeometryAttribute::POSITION || element_type == MESH_VERTEX_ATTRIBUTE ||
      (element_type == MESH_CORNER_ATTRIBUTE && attribute_data_[att_data_id].connectivity_data.no_interior_seams()))
  {
    // Per-vertex attribute reached, use the basic corner table to traverse the
    // mesh.
    MeshAttributeIndicesEncodingData *encoding_data;

    if (use_single_connectivity_ || att->attribute_type() == GeometryAttribute::POSITION)
    {
    ...
    else
    {
      ...
      encoding_data->vertex_to_encoded_attribute_value_index_map.assign(corner_table_->num_vertices(), -1);
    ...
    if (GetEncoder()->options()->GetSpeed() == 0 && att->attribute_type() == GeometryAttribute::POSITION)
    {
  ...
  else
  {
    ...
    typedef DepthFirstTraverser<MeshAttributeCornerTable, AttObserver> AttTraverser;

    MeshAttributeIndicesEncodingData *const encoding_data = &attribute_data_[att_data_id].encoding_data;
    const MeshAttributeCornerTable *const corner_table = &attribute_data_[att_data_id].connectivity_data;

    // Ensure we use the correct number of vertices in the encoding data.
    attribute_data_[att_data_id].encoding_data.vertex_to_encoded_attribute_value_index_map
        .assign(attribute_data_[att_data_id].connectivity_data.num_vertices(), -1);
    ...
    AttObserver att_observer(corner_table, mesh_, traversal_sequencer.get(), encoding_data);
...
template <class TraversalEncoder>
bool MeshEdgebreakerEncoderImpl<TraversalEncoder>::EncodeAttributesEncoderIdentifier(int32_t att_encoder_id)
{
  ...
  if (element_type == MESH_VERTEX_ATTRIBUTE || (element_type == MESH_CORNER_ATTRIBUTE && attribute_data_[att_data_id].connectivity_data.no_interior_seams()))
  {
    // Per-vertex encoder.
...
Status MeshEdgebreakerEncoderImpl<TraversalEncoder>::EncodeConnectivity() {
  ...
  if (corner_table_ == nullptr || corner_table_->num_faces() == corner_table_->NumDegeneratedFaces())
  {
  ...
  // face).
  const uint32_t num_vertices_to_be_encoded = corner_table_->num_vertices() - corner_table_->NumIsolatedVertices();
  EncodeVarint(num_vertices_to_be_encoded, encoder_->buffer());

  const uint32_t num_faces = corner_table_->num_faces() - corner_table_->NumDegeneratedFaces();
  ...
  pos_encoding_data_.vertex_to_encoded_attribute_value_index_map.assign(corner_table_->num_vertices(), -1);
  pos_encoding_data_.encoded_attribute_value_index_to_corner_map.clear();
  pos_encoding_data_.encoded_attribute_value_index_to_corner_map.reserve(corner_table_->num_faces() * 3);
  ...
  const auto num_attribute_data = static_cast<uint8_t>(attribute_data_.size());
  ...
  for (int c_id = 0; c_id < num_corners; ++c_id)
  {
    ...
    if (interior_config)
    {
      ...
      const VertexIndex next_vert_id = corner_table_->Vertex(corner_table_->Next(corner_index));

      const VertexIndex prev_vert_id = corner_table_->Vertex(corner_table_->Previous(corner_index));
      ...
      const CornerIndex opp_id = corner_table_->Opposite(corner_table_->Next(corner_index));
      const FaceIndex opp_face_id = corner_table_->Face(opp_id);

      if (opp_face_id != kInvalidFaceIndex && !visited_faces_[opp_face_id.value()])
      {
  ...
  std::reverse(processed_connectivity_corners_.begin(),processed_connectivity_corners_.end());
  ...
  processed_connectivity_corners_.insert(processed_connectivity_corners_.end(),
    init_face_connectivity_corners.begin(), init_face_connectivity_corners.end());
  ...
  const auto num_encoded_symbols = static_cast<uint32_t>(traversal_encoder_.NumEncodedSymbols());
  ...
  encoder_->buffer()->Encode(traversal_encoder_.buffer().data(),traversal_encoder_.buffer().size());
  return OkStatus();
}
...
{
  auto num_events = static_cast<uint32_t>(topology_split_event_data_.size());
  ...
  if (num_events > 0)
  {
    ...
    for (uint32_t i = 0; i < num_events; ++i)
    {
      ...
      EncodeVarint<uint32_t>(event_data.source_symbol_id - last_source_symbol_id,encoder_->buffer());
      ...
      EncodeVarint<uint32_t>(event_data.source_symbol_id - event_data.split_symbol_id,encoder_->buffer());
    ...
    {
      const TopologySplitEventData &event_data = topology_split_event_data_[i];
      encoder_->buffer()->EncodeLeastSignificantBits32(1, event_data.source_edge);
    }
...
template <class TraversalEncoder>
bool MeshEdgebreakerEncoderImpl<TraversalEncoder>::FindInitFaceConfiguration(FaceIndex face_id, CornerIndex *out_corner) const
{
...
template <class TraversalEncoder>
bool MeshEdgebreakerEncoderImpl<TraversalEncoder>::EncodeConnectivityFromCorner(CornerIndex corner_id)
{
  ...
  while (!corner_traversal_stack_.empty())
  {
    ...
    if (corner_id == kInvalidCornerIndex || visited_faces_[corner_table_->Face(corner_id).value()])
    {
    ...
    while (num_visited_faces < num_faces)
    {
      ...
      if (IsRightFaceVisited(corner_id))
      {
        ...
        if (right_face_id != kInvalidFaceIndex)
          CheckAndStoreTopologySplitEvent(last_encoded_symbol_id_, face_id.value(), RIGHT_FACE_EDGE, right_face_id.value());
        ...
        if (IsLeftFaceVisited(corner_id))
        {
          ...
          if (left_face_id != kInvalidFaceIndex)
            CheckAndStoreTopologySplitEvent(last_encoded_symbol_id_, face_id.value(), LEFT_FACE_EDGE, left_face_id.value());
...
int MeshEdgebreakerEncoderImpl<TraversalEncoder>::EncodeHole(CornerIndex start_corner_id, bool encode_first_vertex)
{
  ...
  VertexIndex start_vert_id = corner_table_->Vertex(corner_table_->Next(corner_id));

  // Get the end vertex of the edge.
  VertexIndex act_vertex_id = corner_table_->Vertex(corner_table_->Previous(corner_id));
...
template <class TraversalEncoder>
CornerIndex MeshEdgebreakerEncoderImpl<TraversalEncoder>::GetRightCorner(CornerIndex corner_id) const
{
...
template <class TraversalEncoder>
CornerIndex MeshEdgebreakerEncoderImpl<TraversalEncoder>::GetLeftCorner(CornerIndex corner_id) const
{
...
template <class TraversalEncoder>
bool MeshEdgebreakerEncoderImpl<TraversalEncoder>::IsRightFaceVisited(CornerIndex corner_id) const
{
  const CornerIndex next_corner_id = corner_table_->Next(corner_id);
...
template <class TraversalEncoder>
bool MeshEdgebreakerEncoderImpl<TraversalEncoder>::IsLeftFaceVisited(CornerIndex corner_id) const
{
  const CornerIndex prev_corner_id = corner_table_->Previous(corner_id);
...
bool MeshEdgebreakerEncoderImpl<TraversalEncoder>::FindHoles()
{
  ...
  for (CornerIndex i(0); i < num_corners; ++i)
  {
    ...
    if (corner_table_->Opposite(i) == kInvalidCornerIndex)
    {
      ...
      VertexIndex boundary_vert_id = corner_table_->Vertex(corner_table_->Next(i));
      ...
      while (vertex_hole_id_[boundary_vert_id.value()] == -1)
      {
        ...
        boundary_vert_id = corner_table_->Vertex(corner_table_->Next(corner_id));
      }
    }
  }
  return true;
...
template <class TraversalEncoder>
int MeshEdgebreakerEncoderImpl<TraversalEncoder>::GetSplitSymbolIdOnFace(int face_id) const
{
...
template <class TraversalEncoder>
void MeshEdgebreakerEncoderImpl<TraversalEncoder>::CheckAndStoreTopologySplitEvent(
    int src_symbol_id, int /* src_face_id */, EdgeFaceName src_edge, int neighbor_face_id)
{
...
bool MeshEdgebreakerEncoderImpl<TraversalEncoder>::InitAttributeData()
{
  ...
  for (int i = 0; i < num_attributes; ++i)
  {
    ...
    if (mesh_->attribute(att_index)->attribute_type() == GeometryAttribute::POSITION)
      continue;

    const PointAttribute *const att = mesh_->attribute(att_index);
    attribute_data_[data_index].attribute_index = att_index;
    attribute_data_[data_index].encoding_data.encoded_attribute_value_index_to_corner_map.clear();
    attribute_data_[data_index].encoding_data.encoded_attribute_value_index_to_corner_map.reserve(corner_table_->num_corners());
    attribute_data_[data_index].encoding_data.num_values = 0;
    attribute_data_[data_index].connectivity_data.InitFromAttribute(mesh_, corner_table_.get(), att);
    ++data_index;
  }
...
// decoder.
template <class TraversalEncoder>
bool MeshEdgebreakerEncoderImpl<TraversalEncoder>::EncodeAttributeConnectivitiesOnFace(CornerIndex corner)
{
  // Three corners of the face.
  const CornerIndex corners[3] = {corner, corner_table_->Next(corner), corner_table_->Previous(corner)};
  ...
  for (int c = 0; c < 3; ++c)
  {
    ...
    for (uint32_t i = 0; i < attribute_data_.size(); ++i)
    {
      if (attribute_data_[i].connectivity_data.IsCornerOppositeToSeamEdge(corners[c]))
...
template class MeshEdgebreakerEncoderImpl<MeshEdgebreakerTraversalPredictiveEncoder>;
