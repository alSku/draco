...
bool MeshEdgebreakerEncoder::InitializeEncoder()
{
  ...
  int selected_edgebreaker_method = options()->GetGlobalInt("edgebreaker_method", -1);

  if (selected_edgebreaker_method == -1)
  {
    if (is_standard_edgebreaker_available && (options()->GetSpeed() >= 5 || !is_predictive_edgebreaker_available || is_tiny_mesh))
  ...
  if (selected_edgebreaker_method == MESH_EDGEBREAKER_STANDARD_ENCODING)
  {
    ...
    {
      buffer()->Encode(static_cast<uint8_t>(MESH_EDGEBREAKER_STANDARD_ENCODING));
  ...
  else if (selected_edgebreaker_method == MESH_EDGEBREAKER_VALENCE_ENCODING)
  {
    ...
    impl_ = std::unique_ptr<MeshEdgebreakerEncoderImplInterface>(
        new MeshEdgebreakerEncoderImpl<MeshEdgebreakerTraversalValenceEncoder>());
  }

  if (!impl_)
    return false;

  return impl_->Init(this);
}

bool MeshEdgebreakerEncoder::GenerateAttributesEncoder(int32_t att_id)
{
  return impl_->GenerateAttributesEncoder(att_id);
}

bool MeshEdgebreakerEncoder::EncodeAttributesEncoderIdentifier(int32_t att_encoder_id)
{
  return impl_->EncodeAttributesEncoderIdentifier(att_encoder_id);
}
...
void MeshEdgebreakerEncoder::ComputeNumberOfEncodedPoints()
{
  ...
  size_t num_points = corner_table->num_vertices() - corner_table->NumIsolatedVertices();
  ...
  if (mesh()->num_attributes() > 1)
  {
    ...
    for (int i = 0; i < mesh()->num_attributes(); ++i)
    {
      ...
      const MeshAttributeCornerTable *const att_corner_table = GetAttributeCornerTable(i);
    ...
    for (VertexIndex vi(0); vi < corner_table->num_vertices(); ++vi)
    {
      ...
      const PointIndex first_point_index = mesh()->CornerToPointId(first_corner_index);
      ...
      while (corner_index != kInvalidCornerIndex)
      {
        ...
        else
        {
          ...
          {
            if (attribute_corner_tables[i]->Vertex(corner_index) != attribute_corner_tables[i]->Vertex(last_corner_index))
            {
              seam_found = true;
...
void MeshEdgebreakerEncoder::ComputeNumberOfEncodedFaces() {
  ...
  set_num_encoded_faces(corner_table->num_faces() - corner_table->NumDegeneratedFaces());
}
