...
int main(int argc, char **argv)
{
  ...
  for (int i = 1; i < argc; ++i)
  {
    ...
    } else if (!strcmp("-qp", argv[i]) && i < argc_check)
    {
      ...
      {
        printf("Error: The maximum number of quantization bits for the position attribute is 30.\n");
        return -1;
      }
    }
    else if (!strcmp("-qt", argv[i]) && i < argc_check)
    {
      ...
      {
        printf("Error: The maximum number of quantization bits for the texture coordinate attribute is 30.\n");
        return -1;
      }
    }
    else if (!strcmp("-qn", argv[i]) && i < argc_check)
    {
      ...
      {
        printf("Error: The maximum number of quantization bits for the normal attribute is 30.\n");
        return -1;
      }
    }
    else if (!strcmp("-qg", argv[i]) && i < argc_check)
    {
      ...
      {
        printf("Error: The maximum number of quantization bits for generic attributes is 30.\n");
        return -1;
      }
    }
  ...
  {
    auto maybe_mesh = draco::ReadMeshFromFile(options.input, options.use_metadata);
  ...
  if (options.tex_coords_quantization_bits < 0)
  {
    ...
    while (pc->NumNamedAttributes(draco::GeometryAttribute::TEX_COORD) > 0)
      pc->DeleteAttribute(pc->GetNamedAttributeId(draco::GeometryAttribute::TEX_COORD, 0));
  }
  ...
  if (options.normals_quantization_bits < 0)
  {
    ...
    while (pc->NumNamedAttributes(draco::GeometryAttribute::NORMAL) > 0)
      pc->DeleteAttribute(pc->GetNamedAttributeId(draco::GeometryAttribute::NORMAL, 0));
  }
  ...
  if (options.generic_quantization_bits < 0)
  {
    ...
    while (pc->NumNamedAttributes(draco::GeometryAttribute::GENERIC) > 0)
      pc->DeleteAttribute(pc->GetNamedAttributeId(draco::GeometryAttribute::GENERIC, 0));
  }
